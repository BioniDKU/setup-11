# YuumeiDKU Windows Update mode - A decicated mode with a lightweight shell just to run Windows Update
...
function Show-WindowTitle($s1) {
	if ($s1 -like "noclose") {
		$host.UI.RawUI.WindowTitle = "Project YuumeiDKU - (c) Bionic Butter | Windows Update mode - DO NOT CLOSE THIS WINDOW"
	} else {
		$host.UI.RawUI.WindowTitle = "Project YuumeiDKU - (c) Bionic Butter | Windows Update mode"
	}
}
Show-WindowTitle
function Show-Branding {
	...
	Write-Host "Windows Update mode" -ForegroundColor Magenta -BackgroundColor Gray
	Write-Host " "
}
...
function Stop-UpdateMode {
	Set-ItemProperty -Path "HKCU:\Software\AutoIDKU" -Name "HikaruMode" -Value 5 -Type DWord -Force
...
function Start-HikaruMusicAndShell {
	...
	if ($hkws -eq $false) {
		...
		if ((Get-ItemProperty -Path "HKCU:\Software\AutoIDKU").DarkSakura -eq 1) {$sakuramode = "DS"} else {$sakuramode = "LS"}
		Copy-Item "$env:SYSTEMDRIVE\Bionic\WinXShell\WinXShell.${sakuramode}.jcfg" -Destination "$env:SYSTEMDRIVE\Bionic\WinXShell\WinXShell.jcfg"
	}
...
function Show-StuckHelp {
	...
	if ($targetcheck -eq 1 -and $edition -notlike "Core") {
		Write-Host " "
		Write-Host "HINT: " -ForegroundColor Magenta -n; Write-Host "This shouldn't happen on Windows 11, but if you got stuck at a Feature Update, try this fix" -ForegroundColor Cyan
		...
		Write-Host "  (say if it's 21H2, change it to 2004, even though this is Windows 11)"
...
Write-Host "Checking for updates again and installing all updates found"; Get-WUList -AcceptAll -Install -NotCategory 'Upgrade' -IgnoreReboot | Out-Null
