...
function Show-WaitTime($wti) {
	for ($wt = $wti; $wt -ge 1; $wt--) {
		$host.UI.RawUI.WindowTitle = "Project YuumeiDKU - (c) Bionic Butter | Windows Update mode - $wt seconds remaining..."
...
if ($wupdated -eq 1) {
	if ($hikareboot -eq 1) {
		...
		Write-Host "Your system have reached the desired UBR." -ForegroundColor Black -BackgroundColor Green -n; Write-Host " If you wish to continue updating, press CTRL+C within $sec seconds." -ForegroundColor Cyan
		Show-WaitTime $sec
